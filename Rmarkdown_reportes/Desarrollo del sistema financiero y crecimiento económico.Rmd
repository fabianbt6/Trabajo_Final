...
- Incluye 2 variables endógenas: PIBr y Cred.
...
```{r}
mod1 <- VAR(dplyr::select(data, pibr_yoy, cred_pib), 
            type = "both", 
            lag.max = 12, ic = "AIC", 
...
ggCcf(x = residuals_mod1$res.pibr_yoy, y = residuals_mod1$res.cred_pib, 
      type = "correlation") + 
...
-   Incluye 3 variables endógenas: PIBr_yoy, Cred y MIF (Margen de Intermediación Financiera).
-   Se definen 5 rezagos, con base al criterio de Akaike.
-   Incluye 3 variables exógenas: Apertura Comercial, Inversión Extranjera Directa, Inflación y el crecimiento interanual del PIB real de USA. 
-   Se incluye 3 variablea dicotómicaa para capturar el valor extremo en la variable PIB real asociado a la pandemia y valores extremos asociados al mif.

```{r}

mod2 <- VAR(data %>% dplyr::select(pibr_yoy, cred_pib, mif),
            type = "both",
            lag.max = 8, ic = "AIC",
            exogen = data %>% dplyr::select(apert_comercial, inflacion, ied, pibusa_yoy, 
                                            d_2Q04, d_4Q07, d_2Q20))
...
ggCcf(x = residuals_mod2$res.pibr_yoy, y = residuals_mod2$res.cred_pib, 
      type = "correlation") + 
  ggtitle("Residuos del modelo PIB vs residuos modelo Cred")

ggCcf(x = residuals_mod2$res.pibr_yoy, y = residuals_mod2$res.mif, 
      type = "correlation") + 
  ggtitle("Residuos del modelo PIB vs residuos modelo MIF")

ggCcf(x = residuals_mod2$res.cred_pib, y = residuals_mod2$res.mif, 
      type = "correlation") + 
  ggtitle("Residuos del modelo PIB vs residuos modelo MIF")
...
-   Incluye 5 variables endógenas: PIBr, Cred, mif (Margen de intermediación financiero), crecimiento interanual del PIB real de USA y la apertura comercial.
-   Se definen 5 rezagos, con base al criterio de Akaike.
-   Se incluye 3 variable dicotómica para capturar el valor extremo en la variable PIB real asociado a la pandemia y para capturar valores extremos en el modelo del mif.

```{r}

mod3 <- VAR(data %>% dplyr::select(pibr_yoy, cred_pib, mif, pibusa_yoy, apert_comercial),
            type = "both",
            lag.max = 8, ic = "AIC",
            exogen = data %>% dplyr::select(d_2Q04, d_4Q07, d_2Q20))
...
apert_a_pib <- irf_comparativo(list(mod3),
                               nombres = "mod3", 
                               impulse_var = "apert_comercial", 
                               resp_var = "pibr_yoy", 
                               acumulado = T)

irf_ggplot(apert_a_pib, 
           "Respuesta del PIB a un impulso la apertura comercial (Efecto Acumulado)",
           "%", 
           intervalo = T)

pibusa_a_pib <- irf_comparativo(list(mod3),
                                nombres = "mod3",
                                impulse_var = "pibusa_yoy",
                                resp_var = "pibr_yoy",
                                acumulado = T)

irf_ggplot(pibusa_a_pib, 
           "Respuesta del PIB a un impulso en en el crecimiento del PIB de USA (Acum.)",
           "%", 
           intervalo = T)
