...
g_pibryoy <- ggplot(data = data, aes(x = fecha, y = log_pibryoy)) + 
  geom_line(size = 0.25) +
  ylab("GDP annual growth") + 
  xlab("date") + 
  theme_classic() + 
  ggtitle("Economic growth") +
  theme_classic() %+replace% 
  theme(plot.title = element_text(size = 7), 
        axis.title.x = element_text(size = 7),
        axis.text.x = element_text(size = 7),
        axis.title.y = element_text(angle = 90, size = 7, margin = margin(t = 0, r = 5, b = 0, l = 0)), 
...
g_credpib <- ggplot(data = data, aes(x = fecha, y = cred_pib)) + 
  geom_line(size = .25) +
  xlab("date") + 
  ylab("In terms of GDP") +
  ggtitle("Financial development") + 
  theme_classic() %+replace% 
  theme(plot.title = element_text(size = 7), 
        axis.title.x = element_text(size = 7),
        axis.text.x = element_text(size = 7),
...
grid.arrange(g_pibryoy, g_credpib, g_apertcomercial, g_pibusa, g_vartc, 
             g_tbp, g_inflacion) 
...
SigVARselect(dplyr::select(data, cpib.cri, des.sf),
          type = "both",
          lag.max = 12,
          exogen = dplyr::select(data, apert_comercial, cpib.usa, inflacion, tbp, vartc, est.d2, est.d3, est.d4))
...
#Tabla Coeficientes modelo modelo VAR======================
...
g_irf_cpib <-  ggplot() +
  geom_line(data = var_dsf_crec_simple, aes(x = periodo, y = irf, colour = "VAR"), size = 0.65) +
  geom_line(data = vecm_dsf_crec_simple, aes(x = periodo, y = irf, colour = "VECM"), size = 0.65) +
  scale_x_continuous(breaks = seq(0, 36, by = 4)) +
  geom_hline(yintercept = 0) +  
  xlab("Period") +
  ylab("economic growth") +
  scale_colour_manual("Model", values = c(VAR = "red", VECM = "darkblue")) +
  theme_bw() %+replace%
  theme(legend.position = "bottom", 
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())

g_irf_des.sf <- ggplot() +
  geom_line(data = var_crec_dsf_simple, aes(x = periodo, y = irf, colour = "VAR"), size = 0.65) +
  geom_line(data = vecm_crec_dsf_simple, aes(x = periodo, y = irf, colour = "VECM"), size = 0.65) +
  scale_x_continuous(breaks = seq(0, 36, by = 4)) +
  geom_hline(yintercept = 0) +  
  xlab("Period") +
  ylab("financial development") +
